class SpecificationsController < ApplicationController
  ...
  def destroy
    @specification = Specification.find(params[:id])
    redirect_to(@specification) and return if params[:cancel]

    respond_to do |format|
    	if @specification.destroy
    		flash[:notice] = 'Specification was successfully destroyed.'
      	format.html { redirect_to(specifications_url) }
      	format.xml  { head :ok }
    	else
    		flash[:notice] = 'Specification was NOT destroyed.'
